...
public:
  ...
  void send_npc_updates(std::vector<std::string> msgs);
...
private:
  ...
  void send_npc_updates();
  void on_write_npc_update(const boost::system::error_code &err, size_t bytes);
  ...
  void handle_send_npc_updates(std::vector<std::string> msgs);

  enum { maxMsg = 1024 };
  char readBuffer_[maxMsg];
  char writeBuffer_[maxMsg];
  char writeOtherBuffer_[maxMsg];
  char writeNpcBuffer_[maxMsg];
  ...
  std::vector<std::string> npcMsgs_;
